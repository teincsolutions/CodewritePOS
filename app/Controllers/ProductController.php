...
class ProductController extends BaseController
{
    ...
    public function edit($id = null)
    {
        ...
        $lastProd = $model->orderBy('id', 'desc')->first();
        ...
        $data = [
            ...
            'brands' => $brandModel->findAll(),
            'stores' => $stores,
            'sku' => $lastProd ? intval($lastProd->sku) + 1 : 1,
        ];

        if ($id) {
            $model = new ProductModel();
            $product = $model->find($id);
            $product->container = (new ContainerModel())->where('id', $product->container_id)->first();
    ...
    public function datatable(): Response
    {
        ...
        if (setting('App.ProductDiffForStore') === 'yes') {
            $model->select('products.id,products.name,products.barcode,products.sku,products.pdiscount,products.description,' .
                'products.category_id,products.brand_id,products.unit_id,products.unit_qty,' .
    ...
    public function search(): Response
    {
        ...
        if (setting('App.ProductDiffForStore') === 'yes') {
            ...
            $model->join('store_products', 'store_products.product_id=products.id');
            $model->where('store_products.store_id', $inputs['store_id'] ?? 0);
        } else $model->select('products.*');


        if (isset($inputs['unit_id']) && $inputs['unit_id'] !== '') {
            $model->where('products.unit_id', $inputs['unit_id']);
        }
        ...
        return $this->response->setStatusCode(200)->setJSON(toDatatableResult($model, $inputs));
    }
    ...
    public function purchase_search(): Response
    {
        ...
        $model->select('products.id,products.name,products.barcode,products.sku,products.category_id,products.brand_id,' .
            ...
            'products.min_qty,purchase_items.unit_price,purchase_items.unit_cost,' .
    ...
    public function sale_search(): Response
    {
        ...
        $model->select('products.id,products.name,products.barcode,products.sku,products.category_id,' .
            ...
            'sales_items.qty, (sales_items.qty-SUM(ifnull(sales_returns_items.qty,0))) as max_qty, sales_items.tax,' .
