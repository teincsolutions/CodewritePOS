...
class SalesController extends BaseController
{
    ...
    {
        $model = new SalesModel();
        $lastItem = $model->where('sales_date', date('ymd', time()))->orderBy('id', 'desc')->first();
    ...
    public function save()
    {
        ...
        $cusModel = new CustomerModel();
        $conStockModel = new ContainerStockModel();
        $cusContainerModel = new CustomerContainerModel();
        ...
        $lastItem = $model->where('sales_date', $inputs['sales_date'])->orderBy('id', 'desc')->first();
        $inputs['invoice'] = $lastItem ? intval($lastItem->invoice) + 1 : date('ymd', strtotime($inputs['sales_date'])) . str_pad('1', 4, '0', STR_PAD_LEFT);
        ...
        if (($inputs['customer_id'] === null || empty($inputs['customer_id']))
            && floatval($inputs['total_amount']) > floatval($inputs['paid'])
        ) return $this->response->setJSON(
            [
                'status' => false,
                'data' => null,
        ...
        try {
            ...
            if ($saved) {
                ...
                $cbuilder1 = $conStockModel->builder();
                $cbuilder2 = $cusContainerModel->builder();
                ...
                foreach ($items as $k => $row) {
                    ...
                    $product = (new ProductModel())->where('id', $items[$k]['product_id'])->first();

                    if ($product && $product->has_container == 1) {
                        $conStockWhere = [
                            'container_id' => $product->container_id,
                            'store_id' => $items[$k]['store_id']
                        ];
 
                        // containers given to customers
                        if ($inputs['type'] === 'customer') {
                            $custContWhere = array_merge($conStockWhere, [
                                'container_id' => $product->container_id,
                                'store_id' => $items[$k]['store_id'],
                                'customer_id' => $inputs['customer_id']
                            ]);
                            if ($cbuilder2->where($custContWhere)->get()->getRowObject()) {
                                $cbuilder2->set('instock', '(instock + ' . $items[$k]['qty'] . ')', false)
                ...
                if ($inputs['customer_id']) {
                    ...
                    if (setting('App.AllowCustomerLimit') === 'yes') {
                        ...
                        if ($balance > $customer->credit_limit && $inputs['payment_status'] === 'due' && $customer->credit_limit !== null)
                            return $this->response->setJSON(
                                [
                                    'status' => false,
                                    'data' => null,
                        ...
                        if (setting('App.LimitSalesDebitDays') === 'yes')
                            if (
                                $days !== null
                                && intval($days) > $customer->credit_limit_days
                                &&  $inputs['payment_status'] === 'due'
                                && $customer->credit_limit_days !== null
                            )
                                return $this->response->setJSON(
                                    [
                                        'status' => false,
