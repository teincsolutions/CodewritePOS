...
class AdjustmentController extends BaseController
{
    ...
    public function edit()
    {
        ...
        $lastItem = $model->where('adj_date', date('ymd', time()))->orderBy('id', 'desc')->first();
    ...
    public function save_reset()
    {
        ...
        try {
            ...
            if ($saved) {
                ...
                $adjustmentItems = [];
                $builder = $stockModel->builder();
                foreach ($stockModel->where('store_id', $inputs['store_id'])->findAll() as $row) {
                    array_push($adjustmentItems, [
                        'adjustment_id' => $id,
                        'product_id' => $row->product_id,
                        'store_id' => $inputs['store_id'],
                        'instock_qty' => $row->instock,
                        'qty' => 0,
                        'unit_cost' => $row->product->unit_cost,
                        'subtotal' => $row->instock * $row->product->unit_cost,
                    ]);
                    $builder->set('instock', 0, false)
                        ->set('updated_at', date('Y-m-d H:i:s'))
                        ->update(null, [
                            'product_id' => $row->product_id,
                            'store_id' =>  $inputs['store_id']
    ...
    public function stock_report_datatable(): Response
    {
        ...
        $builder->select('stock_adjustments.*, concat(users.firstname," ",users.lastname) as username')
