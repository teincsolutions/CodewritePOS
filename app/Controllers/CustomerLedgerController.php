...
class CustomerLedgerController extends BaseController
{
    ...
    public function save()
    {
        ...
        if ($customerLedger) {
            ...
            // if customer returns credit field is updated 
            if ($customerLedger->ledger_type === 'returns' && isset($inputs['debit'])) {
                $saleReturnModel->update($customerLedger->sales_return_id, ['paid' => $inputs['debit']]);
            }
    ...
    public function report_datatable(): Response
    {
        ...
        return $this->response->setJSON(toBuilderDatatableResult($builder, $inputs, function ($item) use ($db) {
            ...
            $item->sale = model('SalesModel')->builder()->where('id', $item->sale_id)->get()->getFirstRow();
            $item->user = model('UserModel')->builder()->where('id', $item->user_id)->get()->getFirstRow();
            $item->sales_return = model('SalesReturnModel')->builder()->where('id', $item->sales_return_id)->get()->getFirstRow();
