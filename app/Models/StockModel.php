...
class StockModel extends Model
{
    ...
    public function getValue()
    {
        if (setting('App.ProductDiffForStore') === 'yes') {
            return
                $this->builder()
                ->join('store_products', 'store_products.product_id=stocks.product_id')
                ->selectSum('(store_products.unit_cost*stocks.instock)', 'total')
                ->get()
                ->getRowObject()
                ->total ?? 0;
        }
        ...
        return
            $this->builder()
            ->join('products', 'products.id=stocks.product_id')
            ->selectSum('(products.unit_cost*stocks.instock)', 'total')
            ->get()
            ->getRowObject()
            ->total ?? 0;
    }
