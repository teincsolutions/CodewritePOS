...
class ContainerModel extends Model
{
    ...
    protected $afterInsert    = ['setRelated'];
    protected $beforeUpdate   = ['setDefaultId'];
    protected $afterUpdate    = ['updateRelated'];
    ...
    protected function setRelated(array $data)
    {
        $id = $this->getInsertID();
        if (isset($data['data']['product_id']) && empty($data['data']['product_id']))
            $data['data']['product_id'] = NULL;

        if (isset($data['data']['product_id']) && !empty($data['data']['product_id'])) {
            (new ProductModel())->update($data['data']['product_id'], [
                'has_container' => 1,
                'container_id' => $id
            ]);
        }

        if ($data['data']['type'] === 'brand' && isset($data['data']['brand_id']) && !empty($data['data']['brand_id'])) {
            (new ProductModel())->where('brand_id', $data['data']['brand_id'])
                ->update(null, ['has_container' => 1, 'container_id' => $id]);
        }

        return $data;
    }

    protected function updateRelated(array $data)
    {
        if (isset($data['data']['product_id']) && empty($data['data']['product_id']))
            $data['data']['product_id'] = NULL;
        ...
        if (isset($data['data']['product_id']) && !empty($data['data']['product_id'])) {
            (new ProductModel())->update($data['data']['product_id'], [
                'has_container' => 1,
                'container_id' => $data['data']['id']
            ]);
        }

        if ($data['data']['type'] === 'brand' && isset($data['data']['brand_id']) && !empty($data['data']['brand_id'])) {
            (new ProductModel())->where('brand_id', $data['data']['brand_id'])
                ->update(null, ['has_container' => 1, 'container_id' => $data['data']['id']]);
        }
    ...
    protected function setInstock($model)
    {
        if (isset($model['data']) && $model['data']) {
            ...
            if ($model['singleton']) {
                $model['data']->inventory = $stockModel->builder()->where('container_id',  $model['data']->id)->get()->getResult();
                ...
                foreach ($model['data'] as $key => $row) {
                    $model['data'][$key]->inventory = $stockModel->builder()->where('container_id', $row->id)->get()->getResult();
