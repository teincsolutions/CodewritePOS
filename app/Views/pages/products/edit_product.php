    ...
    <form action="<?= site_url('products') ?>" class="card post-form" method="post">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-lg-3 col-sm-6 col-12">
                    <div class="form-group">
                        <label>Has a Container</label>
                        <div class="d-flex gap-5">
                            <label class="inputcheck text-capitalize">Yes
                                <input type="radio" name="has_container" value="1" <?= isset($product) ? ($product->has_container == 1 ? 'checked' : '') : '' ?>>
                                <span class="checkmark"></span>
                            </label>
                            <label class="inputcheck text-capitalize">No
                                <input type="radio" name="has_container" value="0" <?= isset($product) ? ($product->has_container == 0 ? 'checked' : '') : 'checked' ?>>
                                <span class="checkmark"></span>
                            </label>
                        </div>
                    </div>
                </div>
                <div id="related-container" class="col-lg-3 col-sm-6 col-12 <?= isset($product) ? (isset($product->container) ? '' : 'd-none') : '' ?>">
                    <div class="form-group">
                ...
                <div class="col-lg-3 col-sm-6 col-12">
                    <div class="form-group">
                        <label>Taxes</label>
                        <select name="taxes[]" class="select2-tax" multiple>
                            <option value=""></option>
                            <?php
                            if (isset($taxes) && isset($product))
                                foreach ($taxes as $row) { ?>
                                <option value="<?= $row->id ?>" <?= in_array($row->id, explode(',', $product->taxes ?? '')) ? 'selected' : '' ?>><?= $row->label; ?></option>
                                <?php }
                            else if (isset($taxes)) {
                                foreach ($taxes as $row) { ?>
                                    <option value="<?= $row->id ?>"><?= $row->label; ?></option>
