...
function printInvoice(result) {
  Swal.fire({
    ...
    if (form && form.hasClass("refresh-page"))
      if (form.data("refresh-url")) location.assign(form.data("refresh-url"));
...
function searchPrediction(item, i) {
  return (`${item.barcode} ${item.sku} ${item.name} ${item.description==''} ${item.unit.label}`).toLocaleLowerCase().includes(
