      ...
      .done((d, s) => {
        ...
        if (d.data.length === 0) {
          ...
          suggestions.filter(searchPrediction).forEach((item, i) => {
            ...
            b.innerHTML =
              item.discontinued == 1
                ? `<span class="d-flex justify-content-between" style="z-index:1000"><del><code>${item.sku}</code> ${item.name}(${item.unit.label})${hasCont} - <i>${info}</i></del>GHS ${unitPrice}</span>`
                : `<span class="d-flex justify-content-between" style="z-index:1000"><span><code>${item.sku}</code> ${item.name}(${item.unit.label})${hasCont} - <i>${info}</i></span>GHS ${unitPrice}</span>`;
