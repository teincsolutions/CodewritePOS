...
$(function () {
  table = $("#dt-adjustments").DataTable({
    ajax: {
      url: baseUrl + "container-adjustments/datatable",
      dataType: "json",
    ...
    columns: [
      ...
      { data: "adj_date", name: "container_stock_adjustments.adj_date" },
      { data: "invoice", name: "container_stock_adjustments.invoice" },
      {
        data: "store",
        name: "container_stock_adjustments.store_id",
        render: function (data, type, row) {
          if (type === "display")
            return data
      ...
      {
        data: "user",
        name: "container_stock_adjustments.user_id",
        render: function (data, type, row) {
          if (type === "display")
      ...
      {
        data: "id",
        name: "container_stock_adjustments.id",
        render: function (data, type, row) {
          if (type === "display") {
            return `<div class="d-flex align-items-center">
                        <a target="_blank" href="${baseUrl}container-adjustments/${data}" class="me-3"><i class="fa fa-eye fa-lg"></i></a>
                        <a hidden class="text-danger" href="javascript:void(0);" onclick="deleteRow(table, ${data}, '${baseUrl}container-djustments')"><i class="fa fa-trash fa-lg"></i></a>
                    </div>`;
          }
          return data;
        },
