let table;
$(function () {
  table = $("#dt-taxes").DataTable({
    ajax: {
      url: baseUrl + "settings/taxes/datatable",
      dataType: "json",
    ...
    columns: [
      ...
      { data: "label", name: "taxes.label" },
      { data: "description", name: "taxes.description" },
      {
        data: "rate",
        render: function (data, type, row) {
      ...
      },
      {
        ...
        render: function (data, type, row) {
          if (type === "display") {
            ...
            return `<div class="me-3 status-toggle d-flex align-items-center"><input type="checkbox" ${
              ["", "checked"][data == "opened" ? 1 : 0]
            } id="user${row.id}" onchange="updateRow(table,{id:${
              row.id
            },status:${
              status[data == "opened" ? 0 : 1]
            }}, '${baseUrl}settings/taxes')" class="check"><label for="user${
              row.id
            }" class="checktoggle">checkbox</label></div>`;
          }
          return data;
        },
      },
      ...
      },
      {
        ...
        render: function (data, type, row) {
          if (type === "display")
            return `<a class="me-3 text-secondary" onclick="editRow('#editpayment',{id:${data},label:'${row.label}', description:'${row.description}',rate:'${row.rate}',status:'${row.status}'},{text:'${row.status}'})" href="javascript:void(0);"><i class="fa fa-edit fa-lg"></i></a>
                <a class="text-danger" href="javascript:void(0);" onclick="deleteRow(table, ${data}, '${baseUrl}settings/taxes')"><i class="fa fa-trash fa-lg"></i></a>`;
          return "";
        },
      },
    ],
