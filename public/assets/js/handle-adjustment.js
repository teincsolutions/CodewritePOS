    ...
    $.get(`${baseUrl}products/search`, searchParams, (d, s) => {
      ...
      if (d.data.length === 0) {
        ...
        d.data.forEach((item, i) => {
          ...
          b.innerHTML =
            item.discontinued == 1
              ? `<span class="d-flex justify-content-between" style="z-index:1000"><del><code>${item.sku}</code> ${item.name}(${item.unit.label}) - <i>${info}</i></del>GHS ${item.unit_cost}|${item.unit_price}</span>`
              : `<span class="d-flex justify-content-between" style="z-index:1000"><span><code>${item.sku}</code> ${item.name}(${item.unit.label}) - <i>${info}</i></span>GHS ${item.unit_cost}|${item.unit_price}</span>`;
