      ...
      (d, s) => {
        ...
        if (d.data.length === 0) {
          ...
          d.data.forEach((item, i) => {
            ...
            info = info.join(",");
            const hasCont =
            item?.has_container == 1
              ? '<span class="badge bg-danger">[C]</span>'
              : "";

            b.innerHTML =
              item.discontinued == 1
                ? `<span class="d-flex justify-content-between" style="z-index:1000"><del><code>${item.sku}</code> ${item.name}(${item.unit.label})${hasCont} - <i>${info}</i></del>GHS ${item.unit_price}</span>`
                : `<span class="d-flex justify-content-between" style="z-index:1000"><span><code>${item.sku}</code> ${item.name}(${item.unit.label})${hasCont} - <i>${info}</i></span>GHS ${item.unit_price}</span>`;
            ...
            b.addEventListener("click", function (e) {
              ...
              let row = ` <tr>
                ...
                item.unit.label
              })${hasCont}</a><span class="badge bg-info">${instock}</span></td>
                                        <td>
